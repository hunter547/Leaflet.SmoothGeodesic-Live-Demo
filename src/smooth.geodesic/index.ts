...
if (typeof window.L !== undefined) {
  L.SmoothGeodesic = L.Path.extend({
    ...
    initialize: function (
      ...
      exponentialMidpointCalculations: number,
      ...
      this._coords = this._generateCoordinates(
        origin,
        destination,
        exponentialMidpointCalculations
      )
          ...
          switch (coord.length) {
            ...
            case 1:
              if (curCommand === 'H') {
                const projectedPoint = map.project(
                  L.latLng(
                    lastCoord[0] as number,
                    coord[0] as unknown as number
                  )
                )
                curPoint = projectedPoint.subtract(map.getPixelOrigin())
    ...
    _curvePointsToPath: function (
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        if (
          typeof point == 'string' &&
          this._isSmoothGeodesicSVGCommand(point)
        ) {
